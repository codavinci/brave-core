...
IN_PROC_BROWSER_TEST_F(DomainBlockTest, NoFetch) {
  ...
  // Temporarily swap out the global URL loader factory and replace it with
  // our own so we can monitor all outgoing network traffic by the entire
  // browser.
  scoped_refptr<network::SharedURLLoaderFactory> old_factory =
  ...
  // Should be zero network traffic since we started monitoring.
  ASSERT_EQ(test_url_loader_factory_.NumPending(), 0);

  // Restore URL loader factory so test environment can clean up properly.
  TestingBrowserProcess::GetGlobal()->SetSharedURLLoaderFactory(
